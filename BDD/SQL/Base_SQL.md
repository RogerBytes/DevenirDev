...
```sql
-- Création de la table ligne_commande (table de liaison).
-- Elle permet d'associer plusieurs plats à une commande, avec une quantité.
...
```sql
-- Données pour la table `ligne_commande`
-- NOTE: Ces données permettent d'illustrer la requête 15 (plats + nombre de commandes).
...
INSERT INTO plat (nom_plat, prix, id_categorie) VALUES (
  'Nouveau plat',
  15.99,
  (SELECT id_categorie FROM categorie WHERE nom_categorie = 'Entrée')
);
```
...
```sql
-- 15. Sélectionne les plats et leur nombre de commandes respectif,
-- triés par nombre de commandes décroissant.
-- Correction: une commande ne contient pas directement id_plat,
-- on passe par la table ligne_commande.
SELECT p.nom_plat AS plat,
       COUNT(lc.id_commande) AS nombre_commandes
FROM plat p
LEFT JOIN ligne_commande lc ON p.id_plat = lc.id_plat
GROUP BY p.id_plat
...
```sql
-- RIGHT JOIN est supporté par MySQL/MariaDB, mais n'existe pas partout (ex: SQLite).
-- Elle inclut tous les enregistrements de la table de droite et les correspondants de la table de gauche.
-- Si votre SGBD ne supporte pas RIGHT JOIN, utilisez un LEFT JOIN avec l'ordre des tables inversé.
SELECT client.nom, commande.date_commande
FROM client
...
```sql
-- Exemple équivalent sans RIGHT JOIN (en inversant l'ordre des tables) :
...
-- Correction: avec les données fournies, COUNT(*) > 5 retourne 0 ligne,
-- donc on met >= 2 pour obtenir un résultat visible.
...
### Utilisation de Fonctions d'Agrégation (SUM, AVG, MIN, MAX)
...
-- NOTE: id_client = 101 n'existe pas forcément dans ce jeu de données.
-- On l'utilise ici comme exemple de syntaxe.
...
-- IMPORTANT :
-- - Les structures IF/THEN/END IF sont exécutables uniquement dans une procédure stockée (ou un trigger),
--   pas en SQL "direct" dans un client classique.
-- - Cet exemple illustre la logique, mais n'est pas exécutable tel quel sans procédure.
